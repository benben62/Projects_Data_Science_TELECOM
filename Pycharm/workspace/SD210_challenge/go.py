...
#     clf = MyEstimator(threshold=threshold, hidden_layer_sizes=(i,), alpha=al)
#     score = np.mean(cross_val_score(clf, X_train, y_train, scoring=scorer))
#     print score
#     if score < minS:
#         minS = score
#         hls = i
#         print 'hls changes to: %s' % i
X_train_1, X_test_1, y_train_1, y_test_1 = train_test_split(X_train, y_train, test_size=0.05, random_state=42)
# parameters = {'learning_rate': ['constant'], 'max_iter': [500],
#               'activation': ['relu']}
clf = MyEstimator(threshold=threshold, hidden_layer_sizes=(hls,), alpha=al, max_iter=200, activation='relu')
clf.fit(X_train_1, y_train_1)
print compute_pred_score(clf.predict(X_test_1), y_test_1)
